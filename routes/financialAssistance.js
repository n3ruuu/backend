...
router.post('/social-pension', (req, res) => {
	...
	const memberQuery = `
        ...
        CONCAT(guardianFirstName, ' ', guardianLastName) AS claimer, guardianRelationship AS relationship
        FROM members
        WHERE id = ?
    `;
	...
	db.execute(memberQuery, [member_id], (err, results) => {
		...
		// Extract member data
		const { control_no, full_name, claimer, relationship } = results[0];
		...
		db.execute(lastSocialPensionIdQuery, [member_id], (err, lastIdResults) => {
			...
			const insertPromises = quarterData.map((data) => {
				const quarter = data.quarter || null;
				const disbursement_date = data.disbursement_date || null;
				const proof = data.proof || null;

				// Insert data and return a Promise
				return new Promise((resolve, reject) => {
					db.execute(
						insertQuery,
						[
						...
						(err) => {
							if (err) {
								console.error('Error inserting record for quarter:', quarter, err);
								reject(err);
							} else {
								resolve();
							}
			...
			Promise.all(insertPromises)
				...
				.catch((err) => {
					...
					res.status(500).json({ message: 'Failed to insert some or all records' });
				});
		});
	});
});
